  ...
  class Idea::SuggestIdeasResolver < BaseResolver
    ...
    def resolve(**args)
      ...
      if same_tag_ideas.length.positive?
        {
          title: '同じタグのアイデア',
          nodes: same_tag_ideas.order('RANDOM()').limit(4)
        }
      # 自分が作成したアイデアが1つ以上ある場合
      elsif idea.same_user_other_ideas.length.positive?
        {
          title: '投稿者の他アイデア',
          nodes: idea.same_user_other_ideas.order('RANDOM()').limit(4)
        }
      else
        {
          title: '他アイデアをのぞいてみる',
          nodes: ::Idea.published.eager_load(%i[idea_tags taggings]).order('RANDOM()').limit(4)
        }
      end
    end
  end
