class UsersController < ApplicationController
  ...
  def show
    ...
    # 後でリファクタする
    @commented_ideas = Kaminari.paginate_array(@user.comments.pluck(:idea_id).uniq.map{ |n| Idea.find_by!(id: n) }.select{ |i| i.user_id != @user.id }).page(params[:comment_ideas]).per(10)
